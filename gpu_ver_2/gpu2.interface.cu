...
__global__ void vectorOrder(double *in, unsigned int *permut, unsigned int dim) { // version per solo Jacobi
    int tx = threadIdx.x;
    int bd = blockDim.x;
...
__global__ void jacobiAlgorithm(double *A, double *I, int NRows, jacobiParameters* var){
    ...
    Diagonal matrix with eigenvalues in the diagonals
    ...
    */
	...
	I_r_tx = I[r * dim + tx];
	    ...
	    __syncthreads();
            ...
            // thread r takes care of updating the elements with row or column equal to r or s
            if(tx==r){
                // use their previous value to compute the new values and update them
		...
		tx_r=0;
            ...
            //else if(tx==s+1 && tx!=dim-1) A[r * dim + tx] = tx_r;
...
class Interface{
    public:
	    ...
	    unsigned int *h_permut = new unsigned int[nCols];
            double hARS;
            int a=0;
            double result;
            double *d_I;
