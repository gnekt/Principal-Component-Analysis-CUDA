...
int main(int argc, char* argv[]) {
    ...
    // Step 3: Find eigenvalues and eigenvectors simultaneously (unified Jacobi)
    double* eigenValues = new double[NCols];
    double* eigenVectors = new double[NCols * NCols];
    int n_eigenvalues = inter.jbEigenvaluesFinder(covarianceMatrix, eigenVectors, eigenValues, NCols, NCols);
    ...
    for (int i = 0; i < NCols; i++) {
        for (int j = 0; j < NCols - 1; j++) {
            outputFile_handler << eigenVectors[j + i * NCols] << ",";
        }
        outputFile_handler << eigenVectors[(NCols - 1) + i * NCols] << endl;
    }
