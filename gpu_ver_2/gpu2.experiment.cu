/*
    Program that generate all the experiment needed for the gpu2 program of PCA
*/
...
/*
    ...
    How to use it:
    ./experiment <string> name of output csv (Eg. kronos.gpu2 )
*/
...
int main(int argc, char* argv[]) {
    ...
    int experimentFeaturesPicking[11][2] = {
        {3109,10}, {3109,15}, {3109,20}, {3109,32},
        {7767,10}, {7767,50}, {7767,100}, {7767,200}, {7767,300}, {7767,400}, {7767,561}
    };
    ...
    if (argc < 2) {
        cerr << "error: insufficient input.\n"
                "usage: " << argv[0] << " <string> name of output csv (Eg. kronos.gpu2 ) \n";
        return 1;
    }
    ...
    for (int experiment = 0; experiment < 11; experiment++) {
        ...
        while (getline(f, line) && i < NRows) {
            ...
            string val;
            stringstream s(line);
            while (getline(s, val, columnSeparatorToken) && j < NCols) {
        ...
        for (int stepIdx = 0; stepIdx < 3; stepIdx++) {
            ...
            for (int iterationTimeIdx = 0; iterationTimeIdx < experimentNumberOfIterationGpu[experiment]; iterationTimeIdx++) {
                if (interTimeIterations[iterationTimeIdx][stepIdx] > elapsedTimeMax)
                    elapsedTimeMax = interTimeIterations[iterationTimeIdx][stepIdx];
            ...
            for (int iterationTimeIdx = 0; iterationTimeIdx < experimentNumberOfIterationGpu[experiment]; iterationTimeIdx++) {
                if (interTimeIterations[iterationTimeIdx][stepIdx] < elapsedTimeMin)
                    elapsedTimeMin = interTimeIterations[iterationTimeIdx][stepIdx];
                elapsedTimeStd += ((interTimeIterations[iterationTimeIdx][stepIdx] - elapsedTimeMean) *
                                   (interTimeIterations[iterationTimeIdx][stepIdx] - elapsedTimeMean)) / experimentNumberOfIterationGpu[experiment];
            }
